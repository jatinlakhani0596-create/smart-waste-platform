...
// ── Bins ──────────────────────────────────────────────────────────────────────
const categories = ["organic", "recyclable", "hazardous", "general", "ewaste", "medical"];
const statuses = ["empty", "partial", "full", "overflow", "damaged"];
export const bins = Array.from({ length: 30 }, (_, i) => {
    const zone = zones[i % zones.length];
    const addrIdx = Math.floor(i / zones.length) % 5;
    const fill = rnd(5, 105); // allow slight overflow
    const clipped = Math.min(fill, 100);
    let status = fill >= 100 ? "overflow" : fill >= 80 ? "full" : fill >= 40 ? "partial" : "empty";
    if (i % 15 === 0)
        status = "damaged";
    return {
...
const truckStatuses = ["on-route", "collecting", "idle", "on-route", "at-depot"];
export const trucks = driverNames.map((name, i) => {
    const zone = zones[i];
    return {
...
const reportStatuses = ["pending", "assigned", "resolved", "resolved", "pending"];
export const reports = Array.from({ length: 20 }, (_, i) => {
    const zone = zones[i % zones.length];
    const cat = categories[i % categories.length];
    const sev = i % 5 === 0 ? "critical" : i % 4 === 0 ? "high" : i % 3 === 0 ? "medium" : "low";
    return {
...
// ── Segregation Records ───────────────────────────────────────────────────────
export const segregationRecords = bins.slice(0, 20).map((bin, i) => {
...
export const householdCompliance = [
...
export const complaints = reports.map((r) => ({
    id: r.id,
    createdAt: r.reportedAt,
    citizenId: r.citizenId,
    ward: r.location.zone === "Central" ? "12" : r.location.zone === "North" ? "07" : "03",
    location: r.location,
    category: (r.description.toLowerCase().includes("medical") ? "mixed-waste" : r.description.toLowerCase().includes("overflow") ? "overflow" : "missed-collection"),
    severity: r.severity,
