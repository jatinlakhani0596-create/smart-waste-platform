...
function routeDistance(stops: GeoPoint[]): number {
  let total = 0;
...
function binPriority(bin: WasteBin): number {
  let score = bin.fillLevel;
  if (bin.status === "overflow")  score += 30;
  if (bin.status === "damaged")   score += 20;
  if (bin.category === "medical" || bin.category === "hazardous") score += 25;
  const daysSinceCollection =
    (Date.now() - bin.lastCollected.getTime()) / (1000 * 60 * 60 * 24);
...
function buildGreedyRoute(
  ...
  let loadKg          = 0;
  const route: WasteBin[]       = [];
  const remaining: WasteBin[]   = [...candidates];
  ...
  while (remaining.length > 0) {
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      if (loadKg + wasteKg > maxLoadKg) continue;
    ...
    if (bestIdx === -1) break;   // truck full or no reachable bins
...
// ── 2-opt improvement ─────────────────────────────────────────────────────────
function twoOpt(route: WasteBin[], origin: GeoPoint): WasteBin[] {
  let improved = true;
  let best     = [...route];
...
// ── Build full CollectionRoute objects ────────────────────────────────────────
export interface RouteOptimizationResult {
  routes:              CollectionRoute[];
  unassignedBins:      WasteBin[];
...
export function optimizeRoutes(minFillThreshold = 50): RouteOptimizationResult {
  ...
  const allRoutes: CollectionRoute[]    = [];
  ...
  for (const truck of availableTrucks) {
    const zoneBins = priorityBins.filter(
      (b) => b.location.zone === truck.location.zone && !assigned.has(b.id)
    );
    if (zoneBins.length === 0) continue;

    const greedyStops = buildGreedyRoute(
      truck.location,
      zoneBins,
      truck.capacityKg - truck.currentLoadKg
    );
    if (greedyStops.length === 0) continue;
    ...
    const stops: RouteStop[] = optimizedStops.map((bin, idx) => {
  ...
  return {
    ...
    summary:
      `${allRoutes.length} optimized routes generated for ${assigned.size} bins. ` +
...
export function simulateRouteProgress(route: CollectionRoute): CollectionRoute {
