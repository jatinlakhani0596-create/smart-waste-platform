...
import type { MunicipalStats, ZoneStat } from "../../data/models.js";
...
export function buildDashboardStats(): MunicipalStats {
  const now         = new Date();
  ...
  const recyclingRate   = parseFloat(
    ((bins.filter((b) => b.category === "recyclable").reduce((s, b) => s + b.fillLevel, 0) /
  ...
  const zoneBreakdown: ZoneStat[] = zoneReports.map((zr) => ({
    ...
    openReports:       reports.filter(
      (r) => r.location.zone === zr.zone && (r.status === "pending" || r.status === "assigned")
    ).length,
  }));
...
export interface DashboardAlert {
  id:       string;
  type:     "overflow" | "report" | "truck" | "segregation" | "sensor";
...
export function getActiveAlerts(): DashboardAlert[] {
