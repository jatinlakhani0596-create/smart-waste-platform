...
export async function classifyIssue(text: string) {
...
export async function routeRecommendation(payload: { ward: string; trafficLevel: string; wasteVolume: number }) {
...
export async function answerWithRag(query: string) {
  ...
  return {
    query,
    documents: docs as AiDocument[],
    answer: `Based on municipal waste rules: ${docs.map((d) => d.title).join("; ")}`,
  };
}

export function interpretComplaint(text: string) {
