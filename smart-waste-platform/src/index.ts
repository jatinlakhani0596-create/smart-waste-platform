...
import {
  ...
  getReportStats,
  getCriticalReports,
} from "./modules/reporting/aiReporter.js";

import {
  optimizeRoutes,
  simulateRouteProgress,
} from "./modules/routing/routeOptimizer.js";

import {
  scanBin,
  getZoneCompliance,
  getSegregationStats,
} from "./modules/segregation/segregationTracker.js";

import {
  buildDashboardStats,
  getActiveAlerts,
  getTruckFleetSummary,
} from "./modules/dashboard/dashboardService.js";
...
function demoReporting() {
  ...
  for (const sub of submissions) {
    ...
    console.log(
      `  ${chalk.gray(report.id)}  ${badge(report.severity)}  ${badge(report.status)}` +
  ...
  if (critical.length > 0) {
    ...
    critical.slice(0, 4).forEach((r) => {
      console.log(
        `   ${chalk.gray(r.id)}  ${badge(r.severity)}  ${chalk.yellow(r.location.address)} — ${r.description.slice(0, 55)}…`
      );
    });
  }
}
...
function demoRouting() {
  ...
  for (const route of result.routes) {
    ...
    route.stops.slice(0, 4).forEach((s) => {
      ...
      console.log(
        `    ${chalk.gray(`#${s.sequence}`.padStart(3))}  ${chalk.cyan(s.binId)}  ` +
  ...
  if (result.unassignedBins.length > 0) {
    ...
    result.unassignedBins.slice(0, 5).forEach((b) => {
      console.log(
        `   ${chalk.gray(b.id)}  fill: ${chalk.white(b.fillLevel + "%")}  ${b.location.address} (${b.location.zone})`
      );
    });
  }
}
...
function demoSegregation() {
  ...
  for (const z of zones) {
    ...
    if (z.topOffenders.length > 0) {
      z.topOffenders.forEach((o) => {
        console.log(
          chalk.dim(`    ↳ ${o.binId}  score: ${o.score}%  contam: ${o.contamination}%`)
        );
      });
    }
  }
  ...
  for (const [cat, data] of Object.entries(stats.categoryBreakdown)) {
    if (data.count > 0) {
      console.log(
        `   ${chalk.cyan(cat.padEnd(14))}  records: ${data.count}  avg compliance: ${chalk.white(data.avgCompliance + "%")}`
      );
    }
  }
}
...
function demoDashboard() {
  ...
  for (const t of fleet) {
    ...
    console.log(
      `  ${chalk.cyan(t.id)}  ${t.driver.padEnd(16)}  ${badge(t.status)}  ` +
